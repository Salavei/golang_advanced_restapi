...
func (d *db) Create(ctx context.Context, user user.User) (string, error) {
	// i think user.User it's mistake. Better CreateUserDTO
...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if err != nil {
		return u, fmt.Errorf("failder to conver hex to objectid. hex: %s", id)
	}
...
func (d *db) Update(ctx context.Context, user user.User) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert user IDD to ObjectID, ID=%s", user.ID)
	}
...
func (d *db) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert user IDD to ObjectID, ID=%s", id)
	}
	filter := bson.M{"_id": objectID}

	result, err := d.collection.DeleteOne(ctx, filter)
	if err != nil {
		return fmt.Errorf("failed to execute equery. error : %s", err)
	}
